...
void writeData(uint8_t value) {
  ...
  for (int i = 7; i >= 0; i--) {
    ...
    digitalWrite(lcdPinout.get_pin(i + 5), bitValue);  // Set clock LOW
  }
  digitalWrite(lcdPinout.WR, HIGH);
  digitalWrite(lcdPinout.WR, LOW);
}
...
void setup() {
  ...
  Address_set(240, 0, 240, 300); // Set address range for the display
}
